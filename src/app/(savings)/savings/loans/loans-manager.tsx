...
export function LoansManager({
  ...
  const live = loans.filter((l) => l.is_active);
  const done = loans.filter((l) => !l.is_active);
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Paid off first. Saving starts once these are gone.
          </p>
        </div>
        ...
        <tr className="bg-slate-50 dark:bg-slate-950/40">
          <td colSpan={7} className="px-4 py-4">
            {payments.length === 0 ? (
              ...
              <div className="max-w-2xl overflow-hidden rounded-xl bg-white ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
