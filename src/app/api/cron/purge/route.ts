 ...
 *
 * Meant to be called by a scheduler (Vercel Cron, or any machine with curl):
 *
 *   GET /api/cron/purge
 *   Authorization: Bearer $CRON_SECRET
 *
 * Deleting data is not something a stray request should be able to trigger, so
 * without CRON_SECRET set this endpoint refuses to run at all rather than
 * running unprotected. The logo files of the deleted businesses go with them:
 * storage is not covered by database cascades.
 */
